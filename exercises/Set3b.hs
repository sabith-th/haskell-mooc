...
sums' i j sum
  | i == j = [sum]
  | otherwise = sum : sums' (i + 1) j (sum + i + 1)
...
sumsOf' [] _ = []
sumsOf' (a : xs) sum = (a + sum) : sumsOf' xs (sum + a)
...
merge xs [] = xs
merge [] ys = ys
merge (a : xs) (b : ys) = if a <= b then a : merge xs (b : ys) else b : merge (a : xs) ys
...
mymaximum _ i [] = i
mymaximum bigger i (a : xs) = if bigger i a then mymaximum bigger i xs else mymaximum bigger a xs
...
map2 :: (a -> b -> c) -> [a] -> [b] -> [c]
map2 f (a : xs) (b : ys) = f a b : map2 f xs ys
map2 f _ _ = []
