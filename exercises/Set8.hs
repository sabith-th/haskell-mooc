...
combine f (Picture f1) (Picture f2) = Picture f'
  where
    f' c = f (f1 c) (f2 c)
...
paintSolid c (Shape sf) (Picture pf) = Picture nf
  where
    nf coord
      | sf coord = c
      | otherwise = pf coord
...
paint (Picture pf) (Shape sf) (Picture bf) = Picture nf
  where
    nf coord
      | sf coord = pf coord
      | otherwise = bf coord
...
instance Transform Blur where
  apply Blur (Picture f) = Picture f'
    where
      f' (Coord x y) = avg (foldr sumC (Color 0 0 0) colors)
        where
          sumC (Color a b c) (Color x y z) = Color (a + x) (b + y) (c + z)
