...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b = if r1 > r2 then a else b
  where
    r1 = measure a
    r2 = measure b
...
whileRight' :: (t -> Either p t) -> Either p t -> p
whileRight' f (Left a) = a
whileRight' f (Right b) = whileRight' f (f b)
...
joinToLength len list = [res | first <- list, last <- list, let res = first ++ last, length res == len]
...
sumRights = sum . map (either (const 0) (* 1))
...
multiCompose fs x
  | null fs = x
  | otherwise = head fs $ multiCompose (tail fs) x
