...
allValues condition (Node a t1 t2) = condition a && allValues condition t1 && allValues condition t2
...
cull val (Node a t1 t2)
  | val == a = Empty
  | otherwise = Node a (cull val t1) (cull val t2)
...
isOrdered (Node a t1 t2) = allValues (< a) t1 && allValues (> a) t2 && isOrdered t1 && isOrdered t2
...
walk (StepL : rest) (Node _ t _) = walk rest t
walk (StepR : rest) (Node _ _ t) = walk rest t
...
set (StepL : rest) val (Node a t1 t2) = Node a (set rest val t1) t2
set (StepR : rest) val (Node a t1 t2) = Node a t1 (set rest val t2)
...
search val tree = go val tree []
  where
    go _ Empty _ = Nothing
    go val (Node a t1 t2) steps
      | a == val = Just steps
      | otherwise = case (go val t1 steps, go val t2 steps) of
        (Nothing, Nothing) -> Nothing
        (Just steps, Nothing) -> Just (StepL : steps)
        (Nothing, Just steps) -> Just (StepR : steps)
