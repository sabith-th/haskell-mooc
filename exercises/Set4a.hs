...
rangeOf (a:xs) = go a a xs
    where
        go max min [] = max - min
        go max min (l:ls)
            | l > max = go l min ls
            | l < min = go max l ls
            | otherwise = go max min ls
...
longest :: Ord a => [[a]] -> [a]
longest = minimumBy customComparator

customComparator :: Ord a => [a] -> [a] -> Ordering
...
incrementKey _ [] = []
incrementKey key ((k,v):vs)
    | k == key = (k,v+1) : incrementKey key vs
    | otherwise = (k,v) : incrementKey key vs
------------------------------------------------------------------------------
...
winner scores player1 player2 = if score1 >= score2 then player1 else player2
    where
        score1 = fromMaybe 0 (Map.lookup player1 scores)
        score2 = fromMaybe 0 (Map.lookup player2 scores)
------------------------------------------------------------------------------
...
freqs = foldr (Map.alter incFreq) Map.empty

incFreq :: Num p => Maybe p -> Maybe p
incFreq (Just f) = Just (f + 1)
incFreq Nothing = Just 1
------------------------------------------------------------------------------
...
transfer from to amount bank
    ...
    | isValidTransfer (Map.lookup from bank) amount = addMoney to amount (deductMoney from amount bank)
    | otherwise = bank


addMoney :: (Ord k, Num a) => k -> a -> Map.Map k a -> Map.Map k a
addMoney to amount = Map.adjust (+ amount) to

deductMoney :: (Ord k, Num a) => k -> a -> Map.Map k a -> Map.Map k a
deductMoney from amount = Map.adjust (\x -> x - amount) from

isValidTransfer :: Ord a => Maybe a -> a -> Bool
isValidTransfer Nothing amount = False
isValidTransfer (Just balance) amount
    | balance >= amount = True
    | otherwise = False
